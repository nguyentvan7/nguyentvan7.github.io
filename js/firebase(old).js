...
firebase.auth().onAuthStateChanged(firebaseUser => {
	if (firebaseUser) {
		...
		var query = firebase.database().ref('/museums').orderByChild('userId').equalTo(userUid);
...
var selects = [document.getElementById('museumSelect'),
document.getElementById('exhibitSelect'), document.getElementById('itemSelect')];
var buttons = [document.getElementById('renameMuseumButton'), document.getElementById('createMuseumButton'),
...
var messages = ['Choose museum name', 'Choose exhibit name', 'Choose item name'];
var messages2 = ['Choose museum name first', 'Choose exhibit name first'];
var titles = ['museum', 'exhibit', 'item', 'user'];
...
function upload() {
	...
	if (file == null || museum == messages[0] || museum == messages2[0] || exhibit == messages[1] || exhibit == messages2[1]
		|| item == messages[2]) {
		ale.className = 'alert alert-danger fade show mt-3';
	...
	Promise.all(dbPromises).then(function () {
		...
		updates['items/item-' + item + '/name'] = selects[2].options[selects[2].selectedIndex].text;
		updates['items/item-' + item + '/id'] = item.toString();
		// Causing issues currently.
		//updates['items/item-' + item + '/user'] = userUid;
		//updates['items/item-' + item + '/type'] = fileTypeString;
		updates['items/item-' + item + '/x'] = x.toString();
		updates['items/item-' + item + '/y'] = y.toString();
		updates['items/item-' + item + '/z'] = z.toString();
		updates['items/item-' + item + '/scale'] = scale.toString();
		updates['exhibits/exhibit-' + exhibit + '/name'] = selects[1].options[selects[1].selectedIndex].text;
		updates['exhibits/exhibit-' + exhibit + '/itemIds/' + item] = true;
		updates['exhibits/exhibit-' + exhibit + '/id'] = exhibit;
		...
		updates['museums/museum-' + museum + '/name'] = selects[0].options[selects[0].selectedIndex].text;
		updates['museums/museum-' + museum + '/exhibitIds/' + exhibit] = true;
		updates['museums/museum-' + museum + '/id'] = museum;
